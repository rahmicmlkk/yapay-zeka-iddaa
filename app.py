...
# Veriyi butona basmadan önce çekiyoruz ki ligleri bulabilelim
data = bugunun_maclarini_getir()

if "response" in data and len(data["response"]) > 0:
    # 1. BUGÜN OYNANAN TÜM LİGLERİ BUL
    bugunun_ligleri = list(set([mac["league"]["name"] for mac in data["response"]]))
    bugunun_ligleri.sort()
    
    # 2. BÜYÜK LİGLER LİSTESİ (Eğer bugün maçları varsa otomatik seçilecekler)
    ana_ligler = ["Süper Lig", "Premier League", "La Liga", "Serie A", "Bundesliga", "Ligue 1", "UEFA Champions League", "UEFA Europa League", "Championship", "Eredivisie"]
    varsayilan_secimler = [lig for lig in ana_ligler if lig in bugunun_ligleri]
    
    # 3. KULLANICIYA LİG SEÇTİRME MENÜSÜ
    st.markdown("---")
    secilen_ligler = st.multiselect(
        "Hedef Ligleri Seçin (Ekleme/Çıkarma yapabilirsiniz):",
        options=bugunun_ligleri,
        default=varsayilan_secimler
    )
    
    # Ekranda kaç maç analiz edileceğini gösterme
    filtrelenmis_mac_sayisi = len([m for m in data["response"] if m["league"]["name"] in secilen_ligler])
    
    if filtrelenmis_mac_sayisi > 0:
        if st.button(f"Seçili Ligleri Analiz Et ({filtrelenmis_mac_sayisi} Maç)"):
            with st.spinner("Seçtiğiniz ligler taranıyor..."):
                
                # Sadece seçilen liglerdeki maçları grupla
                lig_gruplari = {}
                for mac in data["response"]:
                    lig_adi = mac["league"]["name"]
                    if lig_adi in secilen_ligler: # FİLTRE BURADA DEVREYE GİRİYOR
